...
public class Filtfilt {
	 ...
	 *            ÂË²¨Æ÷²ÎÊýA
	 * @param input
	 *            ÒªÂË²¨µÄÊý×é £º ÒªÇóÊÇÒ»Î¬£¬³¤¶ÈÒª´óÓÚBÊý×é¡£
	 * @return ÂË²¨ºóµÄ½á¹û ·µ»ØdoubleÊý×é¡£
	 */
	public static ArrayList<Double> doFiltfilt(ArrayList<Double> B,
		...
		int nfact = 3 * (nfilt - 1);
		if (len <= nfact)
			throw new RuntimeException("ÊäÈëÊýÖµX³¤¶ÈÌ«Ð¡£¬Êý¾Ý×îÉÙÊÇÂË²¨Æ÷½×ÊýµÄÈý±¶");
		resize(B,nfilt, 0);
		resize(A,nfilt, 0);
	...
	public static void filter(ArrayList<Double> B, ArrayList<Double> A,
		...
		if (A.size() == 0)
			throw new RuntimeException("A Êý×éÎª¿Õ£¡");
		boolean flagA = true;
